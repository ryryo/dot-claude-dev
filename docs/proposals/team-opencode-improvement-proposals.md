...
| 7 | [Plan-Exec 統合モード](#提案-7-plan-exec-統合モード) | 低〜中 | 中 |
| 8 | [Delegate モードの明示的採用](#提案-8-delegate-モードの明示的採用) | 低〜中 | 低 |
...
## 提案 7: Plan-Exec 統合モード

### 概要

現行の `dev:team-opencode-plan`（計画）と `dev:team-opencode-exec`（実行）を1つのスキルに統合し、エンドツーエンドのワークフローを提供する。

### 統合フロー

```
/dev:team-opencode
  ├── [PLAN] ストーリー分析 → タスク分解 → レビュー → 承認
  ├── [EXEC] チーム作成 → Wave 実行 → レビューフィードバック
  └── [CLEANUP] 結果集約 → シャットダウン
```
...
```
Q: 実行モードを選択してください。
- plan-only（計画のみ。後で exec を別途実行）
- exec-only（既存計画を選択して実行）
- full（計画→承認→実行を一気通貫）(Recommended)
```

### メリット

- **UX の簡素化**: 1つのスキル呼び出しで完結
- **コンテキストの連続性**: plan → exec 間でリーダーのコンテキストが途切れない
- **操作ミスの削減**: plan 完了後に exec を呼び忘れることがない

### デメリット

- **スキルの肥大化**: SKILL.md が巨大になる（現行で plan 167行 + exec 430行 = 約600行）
- **柔軟性の低下**: 「計画だけ作って後日実行」「計画を複数作って比較」が不便になる
- **関心の分離違反**: 計画と実行は異なる責務。統合は Single Responsibility Principle に反する
- **テスト・デバッグの困難さ**: 問題が plan 側か exec 側かの切り分けが難しくなる

### 採用判断の観点

full モードの利便性は認めつつも、**現行の分離構造を維持し、plan 完了時に「続けて exec を実行しますか？」と AskUserQuestion で提案**する方が、柔軟性を保ちつつ UX を改善できる。スキルの統合よりも、スキル間の連携強化が正しいアプローチ。

---

## 提案 8: Delegate モードの明示的採用

### 概要

公式ドキュメントの Delegate モード（Shift+Tab でトグル）を、team-opencode-exec の Leader に標準適用する。

### 現行の問題

現行スキルの SKILL.md には「リーダーは実装しない」という制約があるが、これはプロンプトレベルの制約であり、技術的な強制力がない。Leader が opencode run を実行してしまう可能性がある。
...
1. TeamCreate
2. Delegate モードを有効化（Leader をコーディネーション専任に制限）
3. Leader は以下のツールのみ使用可能:
   - TeamCreate / TeamDelete
   - TaskCreate / TaskList / TaskGet / TaskUpdate
   - SendMessage
   - AskUserQuestion
   - Read / Glob / Grep（情報収集のみ）
```

### メリット

- **越境防止の強制**: プロンプト制約ではなく、ツールレベルで Leader の実装を防止
- **公式機能の活用**: Agent Teams にネイティブで組み込まれた機能
- **シンプル**: 追加の実装なしで適用可能

### デメリット

- **CLI レベルの機能**: SKILL.md からプログラマティックに Delegate モードを有効化する方法が不明確
- **緊急時の制約**: Leader がエージェント代行する必要がある場合に Delegate モードが邪魔になる
- **実験的機能**: Agent Teams 自体が experimental のため、Delegate モードの安定性は未知数

### 採用判断の観点

現行のプロンプト制約で十分機能している場合は優先度低。**Leader が意図せず実装してしまう事例が頻発する場合に導入**を検討。
...
| **提案 7: Plan-Exec 統合** | 分離の方がメリット大。連携強化で対応 |
| **提案 8: Delegate モード** | 問題が顕在化してから検討 |
