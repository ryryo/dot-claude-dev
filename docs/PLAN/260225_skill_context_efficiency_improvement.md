...
### Phase 3: skill-refactorer 統合・拡張

既存の `skill-refactorer`（7パターン）に Phase 1〜2 で確立した3軸を統合し、`.claude/skills/dev/` に移動する。

- [x] 3.1 スキル仕様の具体化（本セクション）
- [x] 3.2 既存 `skill-refactorer` を `.claude/skills/dev/skill-refactorer/` に移動
- [x] 3.3 SKILL.md + references/patterns.md を再構築（既存7パターン + 新3軸 = 10パターン体制）
- [ ] 3.4 セルフテスト（既存スキルに適用して動作確認）

#### スキル概要

| 項目 | 内容 |
|------|------|
| 名称 | `skill-refactorer`（既存名を継続） |
| 配置先 | `.claude/skills/dev/skill-refactorer/`（シンボリックリンクで各プロジェクトに展開） |
| 入力 | 対象スキルのディレクトリパス |
| 出力 | 改善提案レポート → ユーザー承認後に修正実行 |
| スコープ | 提案 + 承認後に修正実行（提案のみで終わらない） |
| Trigger | スキル整理, SKILL.md圧縮, スキルリファクタ, スキル最適化 |

#### 既存7パターン + 新3軸の統合

既存の skill-refactorer が持つ7パターン:

| # | パターン | 要点 |
|---|----------|------|
| 1 | 圧縮 | 詳細をagents/・references/に委譲、100行未満に圧縮 |
| 2 | DRY化 | 繰り返しを冒頭定義+テーブルに集約 |
| 3 | LLM行動制御 | 「自分でやらない」明示+具体的なツール呼び出しコード |
| 4 | ゲート条件 | 各ステップ末尾にファイル存在チェック |
| 5 | 責務分離 | 詳細をagents/・references/に移す、SKILL.mdはオーケストレータに |
| 6 | 廃止・整理 | 未使用参照・冗長例を削除 |
| 7 | 構造化 | オーケストレータとして再構成（100行未満） |

今回追加する3軸（既存パターンを補完・拡張する位置づけ）:
...
リファレンスやSKILL.md内で出力構造を説明する際、`references/templates/xxx.template.md を参照` と書かない。
init スクリプトで配置済みのファイル（例: `{dir}/PROBLEM_DEFINITION.md`）を Read し、内容を埋めて Write する、という流れで記述する。
テンプレートファイル自体はあくまで init スクリプトの素材であり、実行時の参照先は配置済みファイルとなる。
...
| 新軸 | 関連する既存パターン | 関係 |
|------|---------------------|------|
| 委譲適切性 | #3 LLM行動制御, #5 責務分離 | #3/#5 は「委譲すべき」方向のみ。新軸は「メイン化すべき」判定を追加 |
| テンプレート化 | #1 圧縮, #2 DRY化 | 出力構造の重複をテンプレートに分離する具体的手法を追加 |
| agents/references整理 | #5 責務分離, #6 廃止・整理 | 配置先の判定基準と変換ルールを体系化 |

#### 実行フロー（統合後）

```
1. 対象スキルのディレクトリを Read（SKILL.md, agents/, references/, scripts/, templates/）
2. 既存7パターンでチェック（圧縮・DRY化・LLM行動制御・ゲート条件・責務分離・廃止整理・構造化）
3. 新3軸でチェック:
   a. エージェント委譲の適切性を Step ごとに判定
   b. テンプレート化できる出力ファイルを検出
   c. agents/ と references/ の配置整合性をチェック
4. 改善提案レポートを生成（パターン別に分類）
5. AskUserQuestion でユーザー承認（項目ごとに適用/スキップ選択可）
6. 承認された改善を実行（SKILL.md編集、ファイル移動、テンプレート作成等）
```
