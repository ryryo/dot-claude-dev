...
**ゲート**: アイデアの概要が明確になるまで次に進まない。

### Step 1: 問題定義 → PROBLEM_DEFINITION.md

1. → **エージェント委譲**（problem-definition.md / opus）
2. **Write** で `docs/PROBLEM_DEFINITION.md` を保存

**ゲート**: `docs/PROBLEM_DEFINITION.md` が存在しなければ次に進まない。

### Step 2: 競合分析 → COMPETITOR_ANALYSIS.md

1. → **エージェント委譲**（competitor-analysis.md / sonnet）
2. **Write** で `docs/COMPETITOR_ANALYSIS.md` を保存

**ゲート**: `docs/COMPETITOR_ANALYSIS.md` が存在しなければ次に進まない。

### Step 3: SLCプロダクト仕様 → PRODUCT_SPEC.md

1. → **エージェント委譲**（slc-ideation.md / opus）
2. **Write** で `docs/PRODUCT_SPEC.md` を保存

**ゲート**: `docs/PRODUCT_SPEC.md` が存在しなければ次に進まない。

### Step 4: ユーザー確認

- **AskUserQuestion** で仕様書を確認
  - 「PRODUCT_SPEC.md の内容で進めてよいですか？」
- **dev:storyに進む** → `/dev:story` を案内
- **修正が必要** → 該当ステップに戻って修正
...
| ファイル | Step |
|----------|------|
| `docs/PROBLEM_DEFINITION.md` | Step 1 |
| `docs/COMPETITOR_ANALYSIS.md` | Step 2 |
| `docs/PRODUCT_SPEC.md` | Step 3 |
