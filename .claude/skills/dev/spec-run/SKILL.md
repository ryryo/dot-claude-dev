...
3体のレビューエージェントを **並列** で実行する。各エージェントは信頼度スコア（0-100）で評価し、80以上の指摘のみ報告する。
...
[TDD] ラベル付き Todo に対して、`codex review` → sonnet 3体の 2段階レビューを実行する。
非 [TDD] タスクは従来モードと同じ（sonnet 3体のみ）。

#### Step 2a — codex review（差分バグ検出）
...
#### Step 2b — FIX（codex review 指摘の修正）

1. codex review の指摘内容をプロンプトに含めて `codex exec` で修正を委任（最大1回）
2. 修正後、再度 `codex review --uncommitted` で確認
3. Claude がコミット

#### Step 2c — sonnet 3体並列レビュー

従来モードと同じ3体並列レビューを実行する（品質・正確性・慣例）。
codex review では検出できない仕様適合性・セキュリティ・プロジェクト慣例を補完する。

#### 総合判定

- **全体 PASS**: codex review PASS + 3体すべて PASS
- **全体 FAIL**: いずれかが FAIL → FIX ラウンドへ
...
- `references/agents/tdd-developer.md` — TDD 実装ロール定義（従来モード）
- `references/agents/codex-tdd-developer.md` — Codex TDD 実装プロンプトテンプレート（Codex モード）
- `references/codex-review-instructions.md` — codex review 用レビュー指示テンプレート（Codex モード）
- `agents/reviewer-quality.md` — 品質・設計レビュワー（ベストプラクティス、保守性・可読性）
- `agents/reviewer-correctness.md` — 正確性・仕様適合レビュワー（ストーリー適合性、API・SDK準拠、リスク・前提の検証、セキュリティ）
- `agents/reviewer-conventions.md` — プロジェクト慣例レビュワー（CLAUDE.md準拠、既存パターン、命名規則）
