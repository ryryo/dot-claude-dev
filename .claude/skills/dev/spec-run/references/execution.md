...
Step 5 — TRANSITION   次の Gate 向けにコンテキストを整理し、自動で次の Gate へ進む
```
...
Gate が PASSED した後、次の Gate がある場合に実行する。全 Gate 完了時はスキップ。

### 5-1. compact 実行

次の Gate に焦点を当ててコンテキストを圧縮する。以下の内容を compact のプロンプトに含める:

```
/compact 次の Gate {next_gate_id}: {next_gate_title} の実装に集中する。
保持すべきコンテキスト:
- 仕様書パス（spec.md / tasks.json）
- 次の Gate の契約（Goal / Constraints / AC / Todos）
- プロジェクト全体のアーキテクチャ理解
- 直前の Gate で行った変更のうち、次の Gate に影響するもの
圧縮してよいコンテキスト:
- 完了済み Gate の実装詳細・レビュー詳細
- ファイル内容の逐語的な記憶
```

### 5-2. 次の Gate へ自動継続

compact 完了後、Step 1 に戻り次の Gate の実行を開始する。ユーザーへの確認は不要。
