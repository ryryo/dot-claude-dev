...
全 Gate 通過 + 人間レビュー OK 後に実行する。worktree を base ブランチにマージし cleanup する。

## 前提

- `$WORKTREE_PATH`: setup 時に stdout から記憶済み（セッションをまたいでいる場合は `.worktrees/{slug}/` の絶対パスとして再構築する）
...
2. **base ブランチのルートに cwd を戻す**: `BASE_ROOT="$(git worktree list | head -1 | awk '{print $1}')" && cd "$BASE_ROOT"`
...
1. cwd は base ブランチのリポジトリルート（フェーズ 5 で切り替え済み）
...
- この時点で cwd は base ブランチにいるので、ユーザーは手動で `git merge --abort` や `git reset` で状態を戻せる
