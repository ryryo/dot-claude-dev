...
base ブランチの未コミット変更が worktree 作成後のマージ時にコンフリクトを生まないようにする。
...
5. `cd "$WORKTREE_PATH"` で cwd を worktree に切り替え（以降の全 Bash コマンドは worktree 内で動作）
...
前回中断後に放置された worktree に、base ブランチ側で加えられた最新仕様を取り込む。
...
| ステップ | cwd | 切替コマンド |
|---------|-----|-------------|
| Step 1-4 | base ブランチ | （初期値） |
| フェーズ 1 dirty check | base ブランチ | （変更なし） |
| フェーズ 2 setup-worktree.sh 呼び出し | base ブランチ | （スクリプト内で cd するが戻る、stdout=絶対パス） |
