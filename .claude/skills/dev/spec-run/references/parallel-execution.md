...
4. **VERIFY 実行指示**: IMPL 完了後、Task で `agents/reviewer.md` を起動してレビューを実行
5. **FIX 指示**: FAIL 時は修正して再度 Task(reviewer) を起動（最大3ラウンド）
...
Agent(
  ...
  2. IMPL を実行する
  3. 全変更をコミットする
  4. agents/reviewer.md を Read し、Task で VERIFY を実行する
  5. FAIL があれば修正し、再度 Task(reviewer) を実行する（最大3ラウンド）
  6. Review 結果を返却する
  """
)
```
...
2. Agent（worktree なし）で起動し、マージ結果全体をレビュー
   - 個別レビューでは検出できない統合問題（インターフェース不一致、重複コード等）に注目
3. FAIL → 修正 → 再レビュー
4. PASS → 仕様書に結果を記録
