...
description: |
  ...
  Vercel 環境変数（Production / Development）とローカル env ファイルの両方を
...
**Vercel 環境変数** と **ローカル env ファイル** の両方を即時更新するスキル。
...
| `CLAUDE_WEB_VERCEL_PROJECT_DIR` | `vercel env` 実行時の作業ディレクトリ | ❌ デフォルト: `dashboard/`（`.vercel/project.json` が存在する場所） |
