...
5. **tasks.json 作成**: `references/templates/tasks.template.json` を Read し、`schemaVersion: 3` で Write
   ...
   - `progress: { gatesPassed: 0, gatesTotal: N, currentGate: "<最初のGate ID>", currentGateAC: { passed: 0, total: <最初のGateのAC数> } }`
   ...
   - `metadata.createdAt` (ISO 8601), `metadata.totalGates`, `metadata.totalTodos`
