...
### Step 6: 仕様書ドラフト作成（tasks.json 先行 → spec.md authored → 同期）

tasks.json を軸とし、spec.md は「authored セクション（設計・背景）」+「generated 領域（スクリプト自動生成）」の 2 層構成として作る。

1. `date +%y%m%d` で日付を取得
2. AskUserQuestion で仕様書の slug を確定
3. `mkdir -p docs/PLAN/{YYMMDD}_{slug}` でディレクトリを作成
4. **tasks.json 先行作成**: `references/templates/tasks.template.json` を Read し、構造に従って `docs/PLAN/{YYMMDD}_{slug}/tasks.json` に Write
   - `schemaVersion: 2` を設定
...
5. **spec.md authored 部分作成**: `references/templates/spec-template-dir.md` を Read し、**authored セクションのみ** `docs/PLAN/{YYMMDD}_{slug}/spec.md` に Write
   ...
   - 「タスクリスト」セクションは `<!-- generated:begin -->` と `<!-- generated:end -->` マーカーのみ置き、**内部は空**にする（次 Step で生成される）
   - 実装詳細は **書かない**（tasks.json の impl に格納済み）
