...
- 各 Todo は IMPL 完了後に質的レビューを実行し、全 PASS で Gate 通過

#### レビュー方式

各 Todo のレビューは**質的レビューエージェント**が 6 観点で自律的に評価する（仕様書にチェックリストは書かない）:
...
3. `references/template/spec-template.md` を Read し、その構成に従って `docs/PLAN/{YYMMDD}_{slug}.md` に Write
   - タスクリストは **Gate/Review 形式**で記述（テンプレートの「タスクリスト」セクション参照）
   - 各 Todo に IMPL + Review（検証チェックリスト付き）を含める
...
6. **リスク** — 残存リスクの緩和策が十分か
7. **Review 定義** — 各 Todo に Review が定義されているか
8. **レビュー方式** — 質的レビュー（6 観点自律判断）が正しく記述されているか
...
- [ ] 各 Todo に IMPL + Review（質的レビュー 6 観点）が定義されている
