...
### Step 6: 仕様書ドラフト作成（spec.md authored → tasks.json → generated 同期）

spec.md は「authored セクション（背景・設計）」+「generated 領域（スクリプト自動生成）」の 2 層構成。**背景・設計決定を先に確定させ、その上で impl を書く**という規律を保つため、以下の 3 段で作成する。

1. `date +%y%m%d` で日付を取得
2. AskUserQuestion で仕様書の slug を確定
3. `mkdir -p docs/PLAN/{YYMMDD}_{slug}` でディレクトリを作成
4. **spec.md authored 部分作成**: `references/templates/spec-template-dir.md` を Read し、**authored セクションのみ** `docs/PLAN/{YYMMDD}_{slug}/spec.md` に Write
   - Gate 0（`/dev:spec-run` 参照）・概要・背景・設計決定事項・アーキテクチャ詳細・変更対象ファイルと影響範囲・参照すべきファイル・レビューステータス・残存リスク
   - **Preflight がある場合**: Gate 0 直後に Preflight チェックリストを出力（テンプレートの書式に従う）
   - 「タスクリスト」セクションは `<!-- generated:begin -->` と `<!-- generated:end -->` マーカーのみ置き、**内部は空**にする（Step 6 の最後で自動生成される）
   - 実装詳細は **書かない**（tasks.json の impl に格納する）
5. **tasks.json 作成**: Step 4 で確定した設計決定・アーキテクチャ詳細を参照しながら、`references/templates/tasks.template.json` を Read し、構造に従って `docs/PLAN/{YYMMDD}_{slug}/tasks.json` に Write
   - `schemaVersion: 2` を設定
   ...
   - `todos` 配列に全 Todo を格納。各 Todo に以下を設定:
     ...
     - `dependencies`, `affectedFiles`, `impl`（実装詳細・コード例を含む完全な手順。spec.md の「設計決定事項」「アーキテクチャ詳細」と矛盾しないこと）, `relatedIssues`
...
6. **generated 領域の同期**: Bash で以下を実行し、spec.md の generated 領域を tasks.json から自動生成する

   ```bash
   node .claude/skills/dev/spec-run/scripts/sync-spec-md.mjs docs/PLAN/{YYMMDD}_{slug}/tasks.json
   ```

   スクリプトが依存関係図・Gate/Todo チェックリスト・Review 記入欄を tasks.json から生成し、`<!-- generated:begin/end -->` 内に書き込む。失敗時はエラーを確認して tasks.json の形式を修正する。

   > 補足: Step 5 で tasks.json を Edit/Write した時点で PostToolUse hook (`sync-spec-md-hook.sh`) が自動発火し同じ処理が走るため、明示実行はセーフティネット確認の意味合いが強い。hook が動作している環境ではログ確認だけで済む場合もある。
...
レビュー観点（tasks.json を主軸、spec.md は補助）:
...
6. **リスク** — 残存リスクの緩和策が十分か
7. **Review 構造** — 各 Todo の `steps[]` が impl + review の 2 要素で構成されているか
8. **Gate 0** — spec.md 冒頭に Gate 0（`/dev:spec-run` 参照）が記述されているか
9. **意味的整合性** — tasks.json の `impl` / `affectedFiles` が spec.md の「設計決定事項」「アーキテクチャ詳細」「変更対象ファイルと影響範囲」と矛盾していないか
...
> **注**: Step 6 の最後に `sync-spec-md.mjs` を実行しているため、spec.md の generated 領域は tasks.json と同期済み。`tasks.json` PostToolUse hook（`sync-spec-md-hook.sh`）もセーフティネットとして動作するため、以後 tasks.json を編集すれば spec.md の generated 領域は自動追従する。
...
- `agents/plan-reviewer.md` — 仕様書全体のレビューエージェント指示書（Step 8、構造 + 意味的整合性チェックを含む）
- `.claude/skills/dev/spec-run/scripts/sync-spec-md.mjs` — tasks.json → spec.md の generated 領域を自動生成（Step 6 末尾で明示実行、また tasks.json の Edit/Write で hook が自動発火）
