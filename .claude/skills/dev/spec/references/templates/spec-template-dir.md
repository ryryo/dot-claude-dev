...
[**概念レベル**の Before/After 図やデータフロー図で「実装後の最終形」を描く]
[型定義・関数本体・API 実装コード等のコードレベル詳細は書かない — Gate の Constraints / AC で十分伝わる]
...
[コードベース外の参照資料のみ記載。コードベース内ファイルは Gate の Constraints / affectedFiles で参照する]
[外部参照がない場合はこのセクション自体を省略]
