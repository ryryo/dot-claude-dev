...
description: |
  JavaScript / React のフロントエンドコードを実装・レビュー・リファクタリングするときに、
  ...
  Trigger:
  front-end-design-patterns, frontend design patterns, React設計, JavaScript設計,
  Reactリファクタリング, フロントエンドパフォーマンス改善, bundle改善,
  レンダリング戦略, Reactレビュー
user-invocable: true
---
...
JavaScript / React の設計・実装・レビュー・リファクタリングで、Patterns.dev 由来の参照知識を必要な分だけ読み、対象プロジェクトの既存コードに合わせて適用する。
このスキルはパターン名を増やすための教育スキルではなく、コードベース上の具体的な問題を Inspect / Classify / Select references / Apply or Review / Verify の流れで扱うための実務スキル。

## ゴール

- ユーザーが個別パターンを選ばなくても、現在のプロジェクトに必要な JavaScript / React パターンを判断する。
- 新規 UI 実装、既存コンポーネント整理、重複ロジック抽出、React render 最適化、bundle / loading 改善、レンダリング戦略判断、PR レビューを支援する。
- Patterns.dev 由来の original reference を要約ではなく根拠として読み、既存コードの規約に沿って最小限の変更へ落とす。
- Vue は扱わない。Vue に関する依頼では、このスキルの references を使わない。
...
- 要約だけで実装判断しない。必要なときは `references/react/*.md` または `references/javascript/*.md` の `original-skill-body` を読んでから判断する。
- Vue はスコープ外。Vue 由来の元資料や Vue 用 reference は参照しない。
...
- React / plain JavaScript / Vite / Next.js / Remix などの stack
...
- 新規 UI 機能を、既存コードに合う React / JS 設計で実装したい
- 肥大化した React コンポーネントや重複ロジックをリファクタリングしたい
...
- PR や差分を React / JS 設計・保守性・性能観点でレビューしたい
...
React 固有の話は `references/react/index.md`、framework-agnostic な JavaScript / loading / bundle / web platform の話は `references/javascript/index.md` を入口にする。
