...
### Step 1a: 要件の明確化（clarify プロセス）

1. ユーザーからフィーチャーの要件を聞き取る（引数 or 対話）
   - 何を実現したいか / 背景・動機 / スコープ（含む / 含まない）
2. `.claude/commands/dev/clarify.md` を Read し、プロセスに従って要件を体系的に明確化:
   - 9エリアのチェックリスト（機能/非機能/技術的制約/UI・UX/エッジケース/統合/データ/セキュリティ/パフォーマンス）
   - AskUserQuestion で 2-4問 × 複数ラウンドの反復明確化
   - 完全性チェック → ギャップがあれば繰り返し
3. 必要に応じて Task（Explore）でコードベース調査を行い、現状を把握する

### Step 1b: 前提・リスクの深掘り（dig プロセス、オプショナル）

1. AskUserQuestion で深掘り実行を確認（大規模フィーチャーは推奨、小規模はスキップ可）
2. 実行する場合、`.claude/commands/dev/dig.md` を Read し、プロセスに従う:
   - Step 1a で明確化済みの項目はスキップ
   - 6カテゴリの前提マッピング → 深い調査 → 発見事項統合
3. clarify + dig の出力を統合し、Step 4（構造計画）のインプットとする
