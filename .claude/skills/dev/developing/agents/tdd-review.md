...
**フォールバック**: `USE_OPENCODE=false` 環境変数が設定されているか、OpenCode CLIが利用できない場合は、Step 3のチェックリストベースの手動分析にフォールバック。

### Step 3: チェックリスト分析

OpenCode利用可能時はOpenCode出力と併せて実施。利用不可時はこのチェックリスト単独で分析。
...
OpenCodeの生出力（またはチェックリスト分析結果）を統合し、以下を決定:

1. **判定**: PASS or FAIL
2. **FAIL時の問題リスト**: 各問題の `ファイル:行`、問題の説明、修正推奨
3. **テスト資産の評価**: 保持/簡素化/削除の分類

OpenCode利用可能時は、OpenCodeの指摘をStep 3のチェックリストで確認・補完する形で統合。
