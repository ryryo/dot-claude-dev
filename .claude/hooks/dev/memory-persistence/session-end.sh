#!/bin/bash
# Stop Hook - セッション終了時にTODO.mdのLast Updatedとin-progress-stories.tmpを更新
...
if [ -f "TODO.md" ]; then
  if grep -q "^\*\*Last Updated\*\*:" TODO.md; then
    ...
    echo "[Stop] Updated TODO.md Last Updated: $TIMESTAMP"
  else
...
" TODO.md
    echo "[Stop] Added Last Updated to TODO.md: $TIMESTAMP"
  fi
fi
...
if [ -s "$STORIES_LIST" ]; then
  echo "[Stop] Updated in-progress stories list: $(wc -l < "$STORIES_LIST" | tr -d ' ') stories"
fi
