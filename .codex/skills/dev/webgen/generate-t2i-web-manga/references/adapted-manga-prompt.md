...
このファイルは、LP・Webサイト掲載用の短い漫画画像を生成するためのプロンプト構造と制約を定義する。画像生成時は全文を読み、該当案件の入力変数、原稿、掲載箇所、キャスト、参照画像を埋めて使う。短く要約した別プロンプトへ置き換えない。

このプロンプトでは次を必ず満たす。

1. 入力はLP・Webサイトの原稿、画像計画、掲載箇所とする。
2. 案件の既存キャスト、人物リファレンス、キャラクターシートを優先する。
3. `$imagegen`で実生成し、目視確認と必要な修正生成まで行う。
4. コマ数はストーリー内容に合わせる。ユーザー指定やLP掲載枠がある場合は`panel_count`を優先する。
5. 既定の画像比率はLP差し込み用の`4:3`とし、指定があればそれに従う。
6. 配色は固定せず、対象ページや参照画像になじませる。
...
- `[Output Format]`、`[Visual Style]`、`[Layout Requirements]`、`[Image Context]`、`[Story Context]`、`Mandatory Requirements`、`Roles of Each Character`、`Article Content` を含めているか。
- 1枚の漫画画像になっているか。
