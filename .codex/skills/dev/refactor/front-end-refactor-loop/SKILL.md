...
9. 検証後、変更path、採否、検証結果を記録する。
10. 対象範囲をfreshに読み直して次の新規監査を実行する。
11. 全対象scopeが同じreview対象範囲で`NO_FINDINGS`を返したら最終検証と最終レポートを行う。
...
完了判定に使えるのは、許容した既存差分以外がないことを確認し、対象範囲をfreshに読み直し、指定形式で返した結果だけ。`BLOCKED`、形式崩れ、親セッションの要約、範囲外の一般論は採用しない。
...
- 最新roundの全監査scopeが同じreview対象範囲で`NO_FINDINGS`を返している。
