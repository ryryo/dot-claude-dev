...
`tasks.json` を読み、`schemaVersion === 3` / `gates[]` 配列 / `status` / `reviewChecked` を確認する。v3 でない場合は中断して理由を報告する。

### 3. 実行準備

`spec.md` と `tasks.json` を読み、`gates[]` 全体・`preflight[]`・`todos[].delegation`・`extensions.codex.delegationPolicy` を把握する。

### 4. Cursor Agent の選択

Cursor Agent は都度選択する。利用可否は次で確認する。
...
未ログイン / 契約なし / 最小実行失敗の場合は Codex 単独にフォールバックする。`docs/PLAN` 更新・Gate PASS 判定・最終統合・commit/push は常に main Codex が担う。
...
1. `goal.what` / `goal.why` / `constraints` / `acceptanceCriteria` / `todos` を読む
2. `tdd: true` の Todo は失敗するテストを先に書く
3. `[SIMPLE]` Todo は局所変更として処理する
4. Cursor Agent に委任する場合は [delegation-brief-template.md](../spec-codex/references/delegation-brief-template.md) の形式で詳細プロンプトを作り、write scope と禁止事項を必ず含める
5. 仕様の曖昧さが出た場合だけ止めてユーザーに確認する
