...
userがPLANに従った全実装を求めた場合は、PLAN全体を現在taskのscopeとする。最初の未完了項目から依存順に進め、各PhaseとGate、必要なJourney、証拠、ledgerの状態更新まで、未完了項目がなくなるか次の停止条件に当たるまで継続する。PhaseやGateを一つ終えたこと、作業量が多いこと、通常のテスト失敗や実装上の難しさだけを理由にuserへ返さない。
