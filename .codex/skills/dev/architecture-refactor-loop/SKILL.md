...
- 理由:
- Cursor CLI worker 判定: use | not-use | unavailable
- Cursor CLI worker 判定理由:
- Codex subagent 判定: use | not-use | unavailable
- Codex subagent 判定理由:
- read scope:
- write scope:
- 禁止範囲:
...
委譲判定は、goal 作成時と実行直前の 2 回行う。実行直前に状況が変わっていたら goal file と `PROGRESS_FILE` を更新する。実装直前の判定は独立したステップとして行い、判定記録を書いてからでなければコードを編集しない。

実装前委譲判定記録は、各 goal で必ずこの形で `PROGRESS_FILE` と goal file に残す。
...
- Cursor CLI worker 判定: use | not-use | unavailable
- Cursor CLI worker 判定理由:
- cursor-agent-sprint-cli path check: exists | missing | not-needed
- Codex subagent 判定: use | not-use | unavailable
- Codex subagent 判定理由:
- main Codex が直接持つ判断:
- owner: main-codex | codex-subagent-readonly | cursor-cli-worker | no-delegation
...
Cursor CLI worker 判定の手順:

1. goal の write scope が単一または少数ファイルに閉じるか確認する。
2. architecture 方針、public contract、依存方向の判断が worker 側に漏れないか確認する。
3. acceptance と verification を worker prompt に具体的に書けるか確認する。
4. 他 goal / 他 worker と write scope が競合しないか確認する。
5. 使う候補になった場合だけ、`WORKSPACE/.codex/skills/dev/cursor-agent-sprint-cli` の存在確認と preflight を行う。
6. 使わない場合も `Cursor CLI worker 判定: not-use` と理由を記録する。
...
1. goal file を読む。
2. 委譲判定ゲートを通す。
3. `実装前委譲判定記録` を `PROGRESS_FILE` と goal file に書く。
4. `Cursor CLI worker 判定` が記録されていることを確認する。
5. 実装前に必要な safety check または characterization test を作る。
6. main Codex または選定 worker が実装する。
7. main Codex が diff と write scope を確認する。
8. 必須検証を実行する。
9. Codex 側レビューを行う。
