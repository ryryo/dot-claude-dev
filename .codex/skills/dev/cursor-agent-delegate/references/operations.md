...
preflightが成功したら元のtask errorを見直し、必要なtaskだけ再投入する。失敗した場合は復旧を止め、実装taskをmain Codexへ戻す。Codex subagentは補助workstreamの範囲を維持する。loginが必要な場合や固定modelが利用できない場合は、ユーザーへ報告する。
